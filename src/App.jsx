...
function compressImage(base64, maxSizeKB = 1024) {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      while (result.length > maxSizeKB * 1024 * 1.37 && quality > 0.2) { quality -= 0.1; result = canvas.toDataURL("image/jpeg", quality); }
      resolve(result);
    };
    img.src = base64;
  });
...
const S = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .sa-left{transform:translateX(-50px)}
  .sa-right{transform:translateX(50px)}
  .sa-scale{transform:scale(0.92)}
  .sa-in{opacity:1!important;transform:none!important}
  .sd-1{transition-delay:0.05s}.sd-2{transition-delay:0.12s}.sd-3{transition-delay:0.19s}
  .sd-4{transition-delay:0.26s}.sd-5{transition-delay:0.33s}.sd-6{transition-delay:0.40s}
  ...
  .ls.act{color:var(--accent)}.ls.don{color:var(--a3)}
  ...
  .v-hero.legit .v-glow{background:var(--a3)}.v-hero.fake .v-glow{background:var(--danger)}.v-hero.expired .v-glow{background:var(--warn)}
  ...
  .v-hero.legit .v-eye{color:var(--a3)}.v-hero.fake .v-eye{color:var(--danger)}.v-hero.expired .v-eye{color:var(--warn)}
  ...
  .v-hero.legit .v-title{color:var(--a3)}.v-hero.fake .v-title{color:var(--danger)}.v-hero.expired .v-title{color:var(--warn)}
  ...
  .v-hero.legit .conf-fill{background:var(--a3)}.v-hero.fake .conf-fill{background:var(--danger)}.v-hero.expired .conf-fill{background:var(--warn)}
  ...
  .chk-dot.pass{background:rgba(52,211,153,0.1);color:var(--a3)}.chk-dot.fail{background:rgba(248,113,113,0.1);color:var(--danger)}.chk-dot.warn{background:rgba(251,191,36,0.1);color:var(--warn)}
  .chk-info{flex:1}
  ...
  .chk-badge.pass{background:rgba(52,211,153,0.1);color:var(--a3)}.chk-badge.fail{background:rgba(248,113,113,0.1);color:var(--danger)}.chk-badge.warn{background:rgba(251,191,36,0.1);color:var(--warn)}
  ...
  .rbtn.p{background:var(--accent);color:#020917}.rbtn.p:hover{background:#7dd3fc;transform:translateY(-1px)}
  ...
  .stat.all .stat-v{color:var(--accent)}.stat.ok .stat-v{color:var(--a3)}.stat.bad .stat-v{color:var(--danger)}.stat.exp .stat-v{color:var(--warn)}
  ...
  .h-item{background:var(--surface);border:1px solid var(--border);border-radius:var(--r);padding:16px 20px;display:flex;align-items:center;gap:14px;transition:all 0.25s}
  ...
  .h-vd.legit{background:rgba(52,211,153,0.1);color:var(--a3)}.h-vd.fake{background:rgba(248,113,113,0.1);color:var(--danger)}.h-vd.expired{background:rgba(251,191,36,0.1);color:var(--warn)}
  ...
  .fi-b{background:rgba(56,189,248,0.1);color:var(--accent)}.fi-p{background:rgba(129,140,248,0.1);color:var(--a2)}
  .fi-g{background:rgba(52,211,153,0.1);color:var(--a3)}.fi-r{background:rgba(248,113,113,0.1);color:var(--danger)}
  .fi-y{background:rgba(251,191,36,0.1);color:var(--warn)}.fi-t{background:rgba(45,212,191,0.1);color:#2dd4bf}
  ...
  .h-title .hi{color:#90EE90}
  .h-title .wc{color:var(--accent)}
  .h-title .ex{color:#ff0000}
  ...
  @media(max-width:880px){
    .nav{padding:13px 18px}.nav-links{display:none}.nav-ham{display:flex}
    ...
    .steps-vis{grid-template-columns:1fr 1fr}.steps-vis::before{display:none}
...
function ToastContainer({ toasts }) {
  return <div className="toast-wrap">{toasts.map(t => <div key={t.id} className={`toast ${t.type} ${t.out?"out":""}`}>{t.type==="success"?"✅":t.type==="error"?"❌":"ℹ️"} {t.msg}</div>)}</div>;
}
...
function ErrorBanner({ error, onRetry, onDismiss }) {
  const getMsg = (err) => {
    ...
    if (err.includes("network")||err.includes("fetch")||err.includes("Failed")) return { title:"Network Error", msg:"Could not connect to the AI service. Check your internet connection.", icon:<Ic.Wifi/> };
    if (err.includes("JSON")||err.includes("parse")) return { title:"AI Response Error", msg:"The AI returned an unexpected response. Please try again with a clearer image.", icon:<Ic.Alert/> };
        ...
        <>
          <div className="scan-btns-row">
            <div className="scan-btn-card" onClick={()=>camRef.current?.click()}>
              <div className="scan-btn-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{width:32,height:32}}><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg></div>
            ...
            <div className="scan-btn-card upload-card" onClick={()=>ref.current?.click()}>
              <div className="scan-btn-icon scan-upload-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{width:32,height:32}}><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg></div>
...
function ResultPage({ result, navigate, toast }) {
  ...
  const copyResult = () => {
    ...
    navigator.clipboard.writeText(text).then(()=>toast("Result copied!","success")).catch(()=>toast("Could not copy","error"));
  };
  ...
  return (
    <div className="result-wrap">
      ...
      <div className="det-grid">
        {[
          {l:"Medicine Name",v:result.medicineName,s:"Generic / Brand",d:"0.07s",dir:"sa-left"},
          {l:"Manufacturer",v:result.manufacturer,s:"Company",d:"0.12s",dir:"sa-right"},
          {l:"Batch Number",v:result.batchNo,s:"Production batch",d:"0.17s",dir:"sa-left"},
          {l:"Reg. Number",v:result.regNo,s:"Registration",d:"0.22s",dir:"sa-right"},
          {l:"Mfg. Date",v:result.mfgDate,s:"Made on",d:"0.27s",dir:"sa-left"},
          {l:"Expiry Date",v:result.expiryDate,s:"Use before",d:"0.32s",dir:"sa-right",
            vc:v==="expired"?{color:"var(--danger)"}:v==="legit"?{color:"var(--a3)"}:{}},
        ].map(({l,v:val,s,d,dir,vc={}}) => (
          <div key={l} className={`det-card sa ${dir}`} style={{animationDelay:d}}>
      ...
      <div className="checks-card sa sa-up">
      ...
      <div className="res-btns sa sa-up">
      ...
      <div className="res-btns sa sa-up" style={{marginTop:0}}>
      ...
      <div className="disclaimer sa sa-up"><strong>Disclaimer:</strong> This result is generated by Claude AI for informational purposes only. Always consult a licensed pharmacist or healthcare provider before making decisions about medication.</div>

      {/* WHAT TO DO NEXT */}
      <div style={{marginTop:32,borderTop:"1px solid var(--border)",paddingTop:32}} className="sa sa-up">
        <div style={{marginBottom:20}}>
          <div style={{fontSize:"0.7rem",fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--accent)",marginBottom:6}}>Next Steps</div>
          <h3 style={{fontFamily:"var(--fd)",fontWeight:700,fontSize:"1rem"}}>What Should You Do?</h3>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {(v==="legit" ? [
            {icon:"✅",text:"This medicine appears safe to use as directed by your doctor or pharmacist.",color:"var(--a3)"},
            {icon:"📋",text:"Always follow the prescribed dosage and complete the full course of treatment.",color:"var(--a3)"},
            {icon:"🔄",text:"Re-verify if you purchase the same medicine from a different source in future.",color:"var(--muted)"},
            {icon:"💾",text:"Your scan has been saved to history for future reference.",color:"var(--muted)"},
          ] : v==="fake" ? [
            {icon:"🚫",text:"Do NOT consume this medicine — it may be counterfeit and potentially harmful.",color:"var(--danger)"},
            {icon:"📞",text:"Report the medicine to DRAP: www.dra.gov.pk or call 051-9255800.",color:"var(--danger)"},
            {icon:"🏪",text:"Return to the pharmacy where you purchased it and request a replacement.",color:"var(--warn)"},
            {icon:"📸",text:"Keep the packaging as evidence when reporting to authorities.",color:"var(--muted)"},
          ] : [
            {icon:"⚠️",text:"Do NOT consume this expired medicine — its effectiveness cannot be guaranteed.",color:"var(--warn)"},
            {icon:"🗑️",text:"Dispose of expired medicines safely — do not flush or throw in regular trash.",color:"var(--warn)"},
            {icon:"💊",text:"Visit your pharmacist to get a fresh supply of the same medicine.",color:"var(--muted)"},
            {icon:"📅",text:"Always check expiry dates when purchasing medicines in future.",color:"var(--muted)"},
          ]).map(({icon,text,color},i)=>(
            <div key={i} className={`sa sa-left sd-${i+1}`} style={{display:"flex",gap:12,alignItems:"flex-start",background:"var(--surface)",border:"1px solid var(--border)",borderRadius:"var(--rs)",padding:"14px 16px"}}>
              <span style={{fontSize:"1.1rem",flexShrink:0}}>{icon}</span>
              <span style={{fontSize:"0.8rem",color,fontWeight:300,lineHeight:1.55}}>{text}</span>
            </div>
          ))}
        </div>
      </div>

      {/* VERIFY ANOTHER PROMO */}
      <div style={{marginTop:24,background:"var(--s2)",border:"1px solid var(--border)",borderRadius:"var(--r)",padding:"20px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}} className="sa sa-up">
        <div>
          <div style={{fontFamily:"var(--fd)",fontWeight:700,fontSize:"0.9rem",marginBottom:3}}>Have more medicines to check?</div>
          <div style={{fontSize:"0.76rem",color:"var(--muted)",fontWeight:300}}>Verify all your medicines — protect your family from counterfeit drugs.</div>
        </div>
        <button className="btn btn-p" onClick={()=>navigate("verify")} style={{whiteSpace:"nowrap"}}>Verify Another <Ic.ArrowR/></button>
      </div>
  ...
  return (
    <div className="container-lg">
      ...
      <div className="stats-row">
        {[{k:"all",cls:"all",l:"Total Scans",v:stats.total,dir:"sa-up sd-1"},{k:"legit",cls:"ok",l:"✅ Legit",v:stats.legit,dir:"sa-up sd-2"},{k:"fake",cls:"bad",l:"❌ Fake",v:stats.fake,dir:"sa-up sd-3"},{k:"expired",cls:"exp",l:"⚠️ Expired",v:stats.expired,dir:"sa-up sd-4"}].map(({k,cls,l,v,dir}) => (
          <div key={k} className={`stat ${cls} sa ${dir}`} onClick={()=>setFilter(k)}>
            <div className="stat-v">{v}</div><div className="stat-l">{l}</div>
          </div>
        ))}
      </div>
      {hist.length>0 && (
        <div className="fbar sa sa-up">
      ...
      {filtered.length===0 ? (
        ...
        <div className="h-list">
          {filtered.map((e,i) => (
            <div key={e.id} className="h-item sa sa-left" style={{transitionDelay:`${i*0.05}s`}}>
              ...
              <div className="h-right">
                ...
                <div className="h-ibtns">
                  <button className="icon-btn" onClick={()=>navigate("result",{result:e})}><Ic.Eye/></button>
                  <button className="icon-btn red" onClick={()=>del(e.id)}><Ic.Trash s={13}/></button>
                </div>
              </div>
            </div>
          ))}
      ...
      {hist.length > 0 && (
        <div style={{marginTop:40,paddingTop:36,borderTop:"1px solid var(--border)"}}>
          <div className="sa sa-up" style={{marginBottom:20}}>
            <h3 style={{fontFamily:"var(--fd)",fontWeight:700,fontSize:"1rem",marginBottom:4}}>📊 Your Verification Summary</h3>
            <p style={{fontSize:"0.8rem",color:"var(--muted)",fontWeight:300}}>Based on your {hist.length} scan{hist.length!==1?"s":""} so far</p>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:24}}>
            {[
              {label:"Safety Rate",val:stats.total>0?Math.round((stats.legit/stats.total)*100)+"%":"—",color:"var(--a3)",desc:"of your scans came back legitimate",icon:"🛡️"},
              {label:"Most Recent",val:hist[0]?.verdict?.toUpperCase()||"—",color:hist[0]?.verdict==="legit"?"var(--a3)":hist[0]?.verdict==="fake"?"var(--danger)":"var(--warn)",desc:hist[0]?.medicineName||"No scans yet",icon:"🕐"},
              {label:"Avg Confidence",val:hist.length>0?Math.round(hist.reduce((a,e)=>a+(e.confidence||0),0)/hist.length)+"%":"—",color:"var(--accent)",desc:"average AI confidence score",icon:"🎯"},
            ].map(({label,val,color,desc,icon},i)=>(
              <div key={label} className={`sa sa-up sd-${i+1}`} style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:"var(--r)",padding:"20px 16px",textAlign:"center"}}>
                <div style={{fontSize:"1.4rem",marginBottom:8}}>{icon}</div>
                <div style={{fontFamily:"var(--fd)",fontWeight:800,fontSize:"1.5rem",color,marginBottom:4}}>{val}</div>
                <div style={{fontFamily:"var(--fd)",fontWeight:600,fontSize:"0.72rem",marginBottom:4}}>{label}</div>
                <div style={{fontSize:"0.68rem",color:"var(--muted)",fontWeight:300,lineHeight:1.4}}>{desc}</div>
              </div>
            ))}
          </div>
        </div>
...
function VerifyPage({ navigate, toast }) {
  ...
  const submit = async () => {
    ...
    } catch (e) { setError(e.message||"Something went wrong."); toast("Analysis failed ❌","error"); }
    ...
    <>
      ...
      <div className="container">
        ...
        <div className="mode-grid">
          <div className={`mode-card sa sa-left ${mode==="image"?"sel":""}`} onClick={()=>{setMode("image");setError("");}}>
          ...
          <div className={`mode-card sa sa-right ${mode==="code"?"sel":""}`} onClick={()=>{setMode("code");setError("");}}>
            <div className="mode-chk"><Ic.Check/></div>
...
function FAQ({ q, a }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="faq-item sa sa-up">
  ...
  return (
    <div className="about-wrap">
      ...
      <div className="how-sec">
        <div className="sec-eye sa sa-up">Process</div>
        <h2 className="sec-title sa sa-up">How MediVerify Works</h2>
        ...
        <div className="steps-vis">
          {[{n:1,c:"sc1",t:"Upload or Enter",d:"Upload/Scan a photo of the label or enter the batch/registration code manually",dir:"sa-up sd-1"},
            ...
            <div key={n} className={`step-blk sa ${dir}`}>
              <div className={`step-circ ${c}`}>{n}</div>
              <div className="step-blk-t">{t}</div>
              <div className="step-blk-d">{d}</div>
            </div>
          ))}
        </div>
      </div>
      <div className="feat-sec">
        <div className="sec-eye sa sa-up">Capabilities</div>
        <h2 className="sec-title sa sa-up">What We Check</h2>
        <p className="sec-desc sa sa-up">Every medicine scan runs through multiple AI-powered verification checks.</p>
        <div className="feat-grid">
          {[{ic:<Ic.Brain/>,cls:"fi-p",t:"AI Vision OCR",d:"Reads and extracts all text from medicine packaging images with high accuracy",dir:"sa-left sd-1"},
            {ic:<Ic.Package/>,cls:"fi-b",t:"Batch Verification",d:"Validates batch number formats against known manufacturer patterns",dir:"sa-up sd-2"},
            {ic:<Ic.Zap/>,cls:"fi-y",t:"Expiry Detection",d:"Extracts and validates manufacturing and expiry dates from the label",dir:"sa-right sd-3"},
            {ic:<Ic.Globe/>,cls:"fi-g",t:"Manufacturer Check",d:"Verifies manufacturer names against known registered pharmaceutical companies",dir:"sa-left sd-4"},
            {ic:<Ic.Lock/>,cls:"fi-r",t:"Counterfeit Detection",d:"Identifies inconsistencies in packaging that indicate counterfeit medicine",dir:"sa-up sd-5"},
            {ic:<Ic.History s={26}/>,cls:"fi-t",t:"Scan History",d:"Every scan is saved locally so you can track and review past verifications",dir:"sa-right sd-6"}
          ].map(({ic,cls,t,d,dir})=>(
            <div key={t} className={`feat-card sa ${dir}`}>
              <div className={`feat-ic ${cls}`}>{ic}</div>
      ...
      <div className="faq-sec">
        <div className="sec-eye sa sa-up">FAQ</div>
        <h2 className="sec-title sa sa-up">Frequently Asked Questions</h2>
        <p className="sec-desc sa sa-up">Everything you need to know about MediVerify.</p>
        {[
      ...
      <div className="tech-sec">
        <div className="sec-eye sa sa-up">Built With</div>
      ...
      {/* PAKISTAN DRAP INFO */}
      <div className="how-sec" style={{paddingBottom:70}}>
        <div className="sec-eye sa sa-up">Pakistan Focus</div>
        <h2 className="sec-title sa sa-up">DRAP Registration in Pakistan</h2>
        <p className="sec-desc sa sa-up">MediVerify is designed with Pakistan's drug regulatory framework in mind.</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          {[
            {icon:"🏛️",title:"What is DRAP?",desc:"The Drug Regulatory Authority of Pakistan (DRAP) is responsible for regulating the import, export, manufacturing, and quality of drugs. All legitimate medicines in Pakistan must be registered with DRAP.",color:"var(--accent)",dir:"sa-left sd-1"},
            {icon:"🔢",title:"DRAP Code Format",desc:"Authentic Pakistani medicines carry codes like DRAP-04-Q/1.0.0. These codes encode the drug category, registration year, and batch details. MediVerify understands these formats.",color:"var(--a2)",dir:"sa-right sd-2"},
            {icon:"⚠️",title:"Counterfeit Problem",desc:"Pakistan faces significant challenges with counterfeit medicines. Substandard and falsified medicines cost lives and erode trust in the healthcare system. AI verification helps fight this.",color:"var(--danger)",dir:"sa-left sd-3"},
            {icon:"✅",title:"How MediVerify Helps",desc:"By analyzing packaging, registration codes, batch formats and manufacturer details, MediVerify gives ordinary citizens a powerful tool to verify medicines before consumption.",color:"var(--a3)",dir:"sa-right sd-4"},
          ].map(({icon,title,desc,color,dir})=>(
            <div key={title} className={`feat-card sa ${dir}`} style={{display:"flex",gap:16,alignItems:"flex-start"}}>
              <div style={{fontSize:"1.8rem",flexShrink:0}}>{icon}</div>
              <div>
                <div className="feat-t" style={{color}}>{title}</div>
                <div className="feat-d">{desc}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="feat-sec" style={{paddingBottom:70}}>
        ...
        <div style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:"var(--rl)",padding:"36px",position:"relative",overflow:"hidden"}} className="sa sa-scale">
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:24}}>
            {[
              {label:"OCR Text Extraction",val:"99%",desc:"Accuracy in reading text from clear medicine label photos",color:"var(--a3)"},
              {label:"Language Support",val:"50+",desc:"Languages and scripts supported for medicine label analysis",color:"var(--accent)"},
              {label:"Analysis Speed",val:"<10s",desc:"Average time to complete full medicine verification analysis",color:"var(--a2)"},
            ].map(({label,val,desc,color},i)=>(
              <div key={label} className={`sa sa-up sd-${i+2}`} style={{textAlign:"center",padding:"20px 16px",background:"var(--s2)",borderRadius:"var(--r)",border:"1px solid var(--border)"}}>
                <div style={{fontFamily:"var(--fd)",fontWeight:800,fontSize:"2rem",color,marginBottom:6}}>{val}</div>
                <div style={{fontFamily:"var(--fd)",fontWeight:700,fontSize:"0.82rem",marginBottom:6}}>{label}</div>
                <div style={{fontSize:"0.73rem",color:"var(--muted)",fontWeight:300,lineHeight:1.5}}>{desc}</div>
              </div>
            ))}
          </div>
          <div style={{marginTop:28,padding:"20px",background:"var(--s2)",borderRadius:"var(--rs)",border:"1px solid var(--border)"}} className="sa sa-up">
            <div style={{fontFamily:"var(--fd)",fontWeight:700,fontSize:"0.85rem",marginBottom:8,color:"var(--a2)"}}>🤖 How Claude AI Analyzes Your Medicine</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {["Reads all visible text using computer vision OCR","Identifies registration codes and validates formats","Checks manufacturer names against known databases","Analyzes batch number patterns for authenticity","Validates expiry date format and checks if passed","Assesses packaging consistency for counterfeit signs"].map((item,i)=>(
                <div key={i} style={{display:"flex",gap:8,fontSize:"0.76rem",color:"var(--muted)",fontWeight:300,alignItems:"flex-start"}}>
                  <span style={{color:"var(--a3)",flexShrink:0,marginTop:1}}>→</span>{item}
                </div>
              ))}
            </div>
          </div>
      ...
      {/* SAFETY DISCLAIMER */}
      <div className="faq-sec" style={{paddingBottom:70,maxWidth:900}}>
        <div className="sec-eye sa sa-up">Important</div>
        <h2 className="sec-title sa sa-up">Safety & Limitations</h2>
        <p className="sec-desc sa sa-up">Understanding what MediVerify can and cannot do helps you use it safely.</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          {[
            {icon:"✅",title:"What MediVerify Can Do",items:["Analyze medicine packaging images using AI","Validate registration code formats like DRAP codes","Check expiry dates and flag expired medicines","Identify known manufacturer name patterns","Detect obvious packaging inconsistencies","Store your verification history locally"],color:"var(--a3)",bg:"rgba(52,211,153,0.04)",border:"rgba(52,211,153,0.15)",dir:"sa-left sd-1"},
            {icon:"⚠️",title:"What MediVerify Cannot Do",items:["Connect to DRAP's live official database","Guarantee 100% accuracy in all cases","Replace professional pharmacist advice","Verify medicines with damaged or unclear labels","Detect all forms of sophisticated counterfeiting","Provide legal certification of authenticity"],color:"var(--warn)",bg:"rgba(251,191,36,0.04)",border:"rgba(251,191,36,0.15)",dir:"sa-right sd-2"},
          ].map(({icon,title,items,color,bg,border,dir})=>(
            <div key={title} className={`sa ${dir}`} style={{background:bg,border:`1px solid ${border}`,borderRadius:"var(--r)",padding:"24px"}}>
              <div style={{fontFamily:"var(--fd)",fontWeight:700,fontSize:"0.95rem",color,marginBottom:16}}>{icon} {title}</div>
              <ul style={{listStyle:"none",display:"flex",flexDirection:"column",gap:8}}>
                {items.map((item,i)=>(
                  <li key={i} style={{display:"flex",gap:8,fontSize:"0.79rem",color:"var(--muted)",fontWeight:300,alignItems:"flex-start"}}>
  ...
  return (
    <div style={{position:"relative",zIndex:1}}>
      <div className="home-hero">
        <div>
          <div className="h-eye sa sa-up">AI-Powered Medicine Verification</div>
          <h1 className="h-title sa sa-up">Is Your Medicine <span className="hi">Legit</span>, <span className="wc">Fake</span> or <span className="ex">Expired?</span></h1>
          <p className="h-desc sa sa-up">Upload a photo of the medicine packaging or enter its code. Claude AI instantly checks authenticity, expiry date, and registration status.</p>
          <div className="h-ctas sa sa-up">
      ...
      <div className="home-feats">
        <div className="hf-lbl sa sa-up">Three verdicts. Instant results.</div>
        <div className="hf-grid">
          {[
            {ic:<Ic.Camera s={23}/>,cls:"fi-b",t:"Image Scan",d:"Upload a photo of the medicine's back label. Claude AI reads batch numbers, expiry dates, and registration codes automatically.",pg:"verify",dir:"sa-left sd-1"},
            {ic:<Ic.Code s={23}/>,cls:"fi-p",t:"Code Verification",d:"Enter the registration or batch code manually for instant AI-powered verification and authenticity analysis.",pg:"verify",dir:"sa-up sd-2"},
            {ic:<Ic.History/>,cls:"fi-g",t:"Scan History",d:"Every verification is saved locally. Track all medicines you've checked with filters, stats, and detailed results.",pg:"history",dir:"sa-right sd-3"},
          ].map(({ic,cls,t,d,pg,dir})=>(
            <div key={t} className={`hf-card sa ${dir}`} onClick={()=>navigate(pg)}>
      ...
      <div style={{padding:"0 48px 100px",maxWidth:1100,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="sa sa-scale" style={{background:"linear-gradient(135deg,rgba(56,189,248,0.06),rgba(129,140,248,0.06))",border:"1px solid var(--border2)",borderRadius:"var(--rl)",padding:"60px 44px",textAlign:"center",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:12,flexWrap:"wrap"}}>
            <button className="btn btn-p" onClick={()=>navigate("verify")} style={{padding:"14px 32px",fontSize:"1rem"}}>Verify Medicine Now →</button>
            <button className="btn btn-o" onClick={()=>navigate("about")} style={{padding:"14px 24px"}}>Learn More</button>
          </div>
        </div>
      </div>
